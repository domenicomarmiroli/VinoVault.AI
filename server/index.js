...
const initDb = async () => {
  ...
  try {
    ...
    // 1. Crea la tabella users se non esiste
    await client.query(`
    ...
    // 2. MIGRAZIONE CRITICA: Rendi la password opzionale (nullable) per supportare Google OAuth
    // Questo risolve l'errore "null value in column password violates not-null constraint"
    try {
        await client.query(`ALTER TABLE users ALTER COLUMN password DROP NOT NULL;`);
        console.log("Database Migration: 'password' column is now nullable.");
    } catch (e) {
        // Se la colonna è già nullable o non esiste, ignoriamo l'errore
    }
...
app.post('/api/auth/google', async (req, res) => {
    ...
    try {
        ...
        if (!email_verified) return res.status(401).json({ error: 'Email Google non verificata' });

        console.log(`[GOOGLE AUTH] Processing: ${email}`);
        ...
        if (!user) {
            // REGISTRAZIONE NUOVO UTENTE (Senza password)
            ...
            try {
                ...
                return res.status(500).json({ error: 'Errore durante la creazione del profilo nel database.' });
            }
        } else {
        ...
        return res.status(500).json({ error: 'Autenticazione Google fallita.' });
    }
});

// Altri endpoint (login, register, wines) rimangono invariati...
...
app.post('/api/auth/login', async (req, res) => {
    ...
    try {
        ...
        if (!user || !user.password || !(await bcrypt.compare(password, user.password))) return res.status(400).json({ error: 'Credenziali non valide' });
