      ...
      <div className="bg-white w-full h-[100dvh] md:h-auto md:max-h-[90vh] md:max-w-2xl md:rounded-2xl shadow-2xl flex flex-col overflow-hidden animate-in slide-in-from-bottom duration-300 relative">
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-6 bg-white pb-32 md:pb-6">
            ...
            <div>
                ...
                <h2 className="text-2xl md:text-3xl font-serif font-bold text-gray-900 leading-tight mb-1">{wine.name}</h2>
                <p className="text-lg text-gray-600 font-medium">{wine.producer}</p>
            </div>

            {/* Dati Tecnici */}
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 py-4 border-y border-gray-100 bg-gray-50/50 rounded-xl p-3">
                <div>
                    <span className="block text-[10px] text-gray-400 uppercase tracking-wider">Annata</span>
                    <span className="font-semibold text-gray-800 text-sm">{wine.year}</span>
                </div>
                <div>
                    <span className="block text-[10px] text-gray-400 uppercase tracking-wider">Vitigno</span>
                    <span className="font-semibold text-gray-800 text-sm">{wine.grape || 'N/D'}</span>
                </div>
                <div>
                    <span className="block text-[10px] text-gray-400 uppercase tracking-wider">Regione</span>
                    <span className="font-semibold text-gray-800 text-sm truncate">{wine.region || 'N/D'}</span>
                </div>
                <div>
                    <span className="block text-[10px] text-gray-400 uppercase tracking-wider">Alcol</span>
            ...
            <div className="space-y-3">
                <h3 className="text-lg font-serif font-bold text-gray-900 flex items-center gap-2">
                    <span className="w-1 h-5 bg-wine-600 rounded-full"></span>
                    Consigli del Sommelier
                </h3>
                
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <div className="bg-stone-50 p-3 rounded-xl border border-stone-100">
                        <div className="flex items-center gap-2 mb-1 text-wine-800 font-bold text-xs uppercase">
                        ...
                        <p className="text-xs text-gray-600 italic leading-relaxed">"{wine.servingAdvice}"</p>
                    </div>

                    <div className="bg-stone-50 p-3 rounded-xl border border-stone-100">
                        <div className="flex items-center gap-2 mb-1 text-wine-800 font-bold text-xs uppercase">
                            <BoxIcon className="w-4 h-4" /> Conservazione
                        </div>
                        <p className="text-sm text-gray-600 mb-1"><span className="font-semibold">Temp:</span> {wine.storageTemp}</p>
                        <p className="text-xs text-gray-600 italic leading-relaxed">"{wine.storageAdvice}"</p>
                    </div>
                </div>
            </div>
