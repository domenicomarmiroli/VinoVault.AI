...
const AnalyticsView: React.FC<AnalyticsViewProps> = ({ inventory, history, onLogout, isPremium, onAiUsed }) => {
  ...
  const typeCounts = inventory.reduce((acc, w) => { acc[w.type] = (acc[w.type] || 0) + w.quantity; return acc; }, {} as Record<string, number>);
  const regionCounts = inventory.reduce((acc, w) => { const region = w.region || 'Sconosciuta'; acc[region] = (acc[region] || 0) + w.quantity; return acc; }, {} as Record<string, number>);
  ...
  const handleOpenReport = async () => {
      ...
      if (inventory.length <= 5) { alert("Add more wines first"); return; }
      ...
      try {
          const report = await generateCellarReport(inventory, history, language);
      ...
      <div className="flex-1 overflow-y-auto p-4 pb-24 space-y-6">
        ...
        <div className="bg-gradient-to-br from-gray-900 to-gray-800 rounded-2xl p-6 text-white shadow-xl relative overflow-hidden">
             ...
             <div className="flex justify-between items-end mb-6">
                 <div><span className="text-4xl font-serif font-bold block">€{totalMarketValue.toFixed(0)}</span><span className="text-xs text-gray-400">{t('estimated_value')}</span></div>
                 <div className="text-right z-10"><span className={`text-xl font-bold block ${roi >= 0 ? 'text-green-400' : 'text-red-400'}`}>{roi > 0 ? '+' : ''}{roi.toFixed(1)}%</span><span className="text-xs text-gray-400">{t('roi')}</span></div>
             </div>
             <div className="grid grid-cols-2 gap-4 border-t border-gray-700 pt-4">
                 <div><span className="block text-xl font-bold text-gray-200">€{averagePurchasePrice.toFixed(2)}</span><span className="text-[10px] text-gray-400 uppercase font-bold">{t('avg_price')}</span></div>
             ...
             <div className="space-y-4">
                 {sortedRegions.map(([region, count], idx) => (
                     <div key={region} className="relative">
                         ...
                         <div className="h-1.5 bg-gray-100 rounded-full overflow-hidden ml-7"><div className="h-full bg-wine-600 rounded-full opacity-80" style={{ width: `${(count / totalBottles) * 100}%` }}></div></div>
                     </div>
                 ))}
             </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
            ...
            <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-200 text-center flex flex-col items-center justify-center">
                 ...
                 <span className="block text-2xl font-bold text-gray-900">€{totalConsumedValue.toFixed(0)}</span>
            ...
            <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-200 text-center col-span-2 flex items-center justify-between px-6">
                ...
                <span className="block text-3xl font-bold text-wine-800">€{averageConsumedPrice.toFixed(2)}</span>
            </div>
        </div>
      </div>
