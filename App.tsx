...
const AppContent: React.FC = () => {
  ...
  const handleLogin = (newToken: string, userEmail: string) => {
      ...
      try {
        ...
        if (payload.role === 'restaurant') setActiveTab('restaurant_management');
    ...
    const fetchData = async () => {
      try {
        ...
        if (profileRes.ok) {
            ...
            if (profile.role === 'restaurant') setActiveTab('restaurant_management');
        }
  ...
  // Tabs filtering by role
  const getTabs = () => {
      if (userRole === 'restaurant') {
          return [
              { id: 'shop', icon: ShopIcon, label: t('nav_shop') },
              { id: 'sommelier', icon: ChefIcon, label: t('nav_sommelier') },
              { id: 'restaurant_management', icon: RestaurantIcon, label: t('nav_my_restaurant') }
          ];
      }
      return [
  ...
  if (!token) {
    ...
    return <LandingPage onStart={() => setShowAuth(true)} onOpenGuide={(s) => window.history.pushState({}, '', `/guida/${s}`)} />;
  }
  ...
  return (
    <div className="flex flex-col h-full w-full md:max-w-md mx-auto bg-white shadow-2xl overflow-hidden md:border-x md:border-gray-200">
      <main className="flex-1 overflow-hidden relative">
        {userRole === 'restaurant' ? (
            <>
                ...
                <div className={`absolute inset-0 transition-opacity ${activeTab === 'restaurant_management' ? 'opacity-100 z-10' : 'opacity-0 z-0'}`}><RestaurantCellarView onLogout={handleLogout} /></div>
            </>
        ) : (
            <>
                <div className={`absolute inset-0 transition-opacity ${activeTab === 'inventory' ? 'opacity-100 z-10' : 'opacity-0 z-0'}`}><InventoryView wines={wines} locations={locations} onAddWine={() => {}} onUpdateWine={() => {}} onConsume={() => {}} onDelete={() => {}} onAddLocation={() => {}} onDeleteLocation={() => {}} onLogout={handleLogout} onAiUsed={() => {}} isPremium={userPremium} /></div>
                <div className={`absolute inset-0 transition-opacity ${activeTab === 'shop' ? 'opacity-100 z-10' : 'opacity-0 z-0'}`}><ShopView inventory={wines} onLogout={handleLogout} onAddToInventory={() => {}} onAiUsed={() => {}} isPremium={userPremium} /></div>
                <div className={`absolute inset-0 transition-opacity ${activeTab === 'sommelier' ? 'opacity-100 z-10' : 'opacity-0 z-0'}`}><SommelierView inventory={wines} onLogout={handleLogout} onAiUsed={() => {}} onConsume={() => {}} /></div>
      ...
      <nav className="bg-white border-t border-gray-200 flex justify-between px-2 pb-safe z-50">
        {getTabs().map(tab => (
